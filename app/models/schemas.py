...
6-provider failover: Groq -> Claude -> GPT-4o -> Gemini -> Command R+ -> Mistral
...
class EliteLLMService:
    ...
    Priority: Groq -> Claude Sonnet 4 -> GPT-4o -> Gemini 2.0 Flash -> Command R+ -> Mistral Large
    ...
    async def chat(
        ...
        provider: LLMProvider = LLMProvider.groq,  # Default changed to Groq
    ...
    def _build_chain(self, preferred_provider, preferred_model):
        all_providers = [
            (LLMProvider.groq,      self._groq,      "Groq (Llama 3 70B)", "llama-3.3-70b-versatile", settings.GROQ_API_KEY),
    ...
    async def _no_keys_error(self, *args, **kwargs):
        raise Exception(
            "No API keys configured. Add GROQ_API_KEY or ANTHROPIC_API_KEY in Railway Variables."
        )

    async def _groq(self, messages, system, model, max_tokens, temperature):
        """Groq provider integration — ultra-fast inference."""
