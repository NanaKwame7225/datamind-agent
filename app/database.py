...
async def _ensure_indexes(db):
    ...
    try:
        ...
        await db.ats_jobs.create_index([("user_id", 1), ("created_at", -1)])
        await db.ats_jobs.create_index([("workspace_id", 1), ("created_at", -1)])
        await db.ats_candidates.create_index([("job_id", 1), ("score", -1)])
