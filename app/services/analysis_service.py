"""
DataMind Agent — Elite Analysis Service
Every claim grounded in actual data computation.
Every finding includes evidence, impact rank, confidence score,
segmentation, uncertainty scoring, and self-audit.
"""
...
class EliteDataAnalysisService:
    ...
    def clean_data(self, df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
        ...
        report["steps"].append({
            "step": "Whitespace strip", "library": "Pandas",
        ...
        for col in df.select_dtypes(include="object").columns:
            ...
            try:
                ...
                pass
        report["steps"].append({
            "step": "Type inference", "library": "Pandas",
        ...
        report["steps"].append({
            "step": "Duplicate removal", "library": "Pandas",
        ...
        for col in numeric_cols:
            ...
            if n_missing > 0:
                ...
                imputed[col] = {
                    "missing_count": n_missing, "missing_pct": pct_missing,
                    "filled_with": "median", "median_value": round(float(median_val),4),
        ...
        report["steps"].append({
            "step": "Missing value imputation", "library": "Pandas + NumPy",
        ...
        report["steps"].append({
            "step": "Outlier winsorisation", "library": "Pandas + NumPy",
        ...
        for col in df.select_dtypes(include="number").columns:
            try:
                ...
                pass
        report["steps"].append({
            "step": "Skewness correction", "library": "NumPy + SciPy",
    ...
    def elite_analyse(self, df: pd.DataFrame, query: str, industry: str) -> dict:
        ...
        for f in result["findings"]:
            ...
            if col not in impact_scores or f["impact_score"] > impact_scores[col]["score"]:
                impact_scores[col] = {
                    ...
                    "confidence": f["confidence"], "primary_issue": f["type"],
                    "reason": f["title"],
                }
        ...
        if n_rows < 30:
            uncertainties.append({
                ...
                "detail": f"Only {n_rows} rows. Statistical conclusions may not generalise — patterns could be coincidental.",
        ...
        if n_rows < 100:
            uncertainties.append({
                ...
                "detail": f"{n_rows} rows is below the recommended 100 for robust analysis.",
                "affected_analyses": ["segmentation","forecasting"],
                "confidence_adjustment": -0.1,
                "recommendation": "Analysis is indicative — validate with more data.",
            })
        for col, info in result["distributions"].items():
            if info["missing_pct"] > 20:
                uncertainties.append({
                    "issue": f"High missing data in {col.replace('_',' ')}",
                    "detail": f"{info['missing_pct']}% of values missing. Imputation may have introduced bias.",
                    "affected_analyses": [f"any analysis involving {col}"],
                    "confidence_adjustment": -0.15,
                    "recommendation": f"Investigate why {col.replace('_',' ')} is missing.",
                })
        if not cat_cols:
            uncertainties.append({
                "issue": "No segmentation variables",
                "detail": "No categorical columns available. Cannot segment — insights are averages that may hide variation.",
                "affected_analyses": ["segmentation analysis"],
                "confidence_adjustment": -0.05,
                "recommendation": "Add region, department, or category columns to enable segment-level insights.",
            })
        ...
        result["self_audit"] = [
            {
                ...
                "assumptions": [
                    ...
                    "Missing values are missing at random, not systematically",
                    "Time ordering of rows reflects actual chronological sequence",
            ...
            },
            {
                ...
                "risks": [
                    "Anomalies flagged could be legitimate peaks — context needed from the business",
                    "Trends may be seasonal patterns misread as directional — more periods needed to confirm",
                    "Correlations may be driven by a third hidden variable",
                    "Segment differences may reflect data quality variation per segment, not true performance",
                ],
            },
            {
                ...
                "missing_context": [
                    ...
                    "External factors (market events, seasonal periods) that explain spikes",
        ...
        result["data_grounding"] = {
            ...
            "total_anomalies_found": sum(
                f.get("evidence",{}).get("anomaly_count",0)
                for f in result["findings"] if f["type"]=="anomaly"
            ),
    ...
    def cluster(self, df: pd.DataFrame, features: list, n_clusters: int = 4) -> dict:
        ...
        return {
            "model": "K-Means", "n_clusters": n_clusters,
