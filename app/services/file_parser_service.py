...
class FileParserService:
    ...
    def inspect(self, file_bytes: bytes, filename: str) -> dict:
        ...
        try:
            if ext in (".xlsx", ".xlsm", ".xls"):
                try:
                    ...
                    return {"success": False,
                            "error": "Excel support requires openpyxl (.xlsx) or xlrd (.xls) on the server.",
                            "hint": "Add openpyxl and xlrd to requirements.txt and redeploy.",
                            "detail": str(e)}
                sheets = []
                for name in xls.sheet_names:
                    ...
                    sheets.append({
                        "name": name,
                        "rows": len(full),
                        "columns": list(full.columns.astype(str))[:30],
                        "preview": json.loads(full.head(5).where(pd.notna(full.head(5)), None).to_json(orient="records")),
                    })
                if not sheets:
                    return {"success": False, "error": "Could not read any sheet from this workbook."}
            ...
            logger.error(f"Missing library during inspect: {e}")
            return {"success": False, "error": str(e),
                    "hint": "A required parsing library is not installed on the server."}
        except Exception as e:
    ...
    def _parse_excel(self, file_bytes: bytes, sheet_name: str = None) -> dict:
        try:
            ...
            raise ImportError(
                ...
                f"Add them to requirements.txt. Original error: {e}"
            )
        sheets = xls.sheet_names
        if not sheets:
            return {"success": False, "error": "This workbook contains no sheets."}
    ...
    def _pdf_table_inventory(self, file_bytes: bytes) -> dict:
        ...
        try:
            ...
            raise ImportError(
                "PDF support requires pdfplumber. Add 'pdfplumber>=0.11.0' to requirements.txt and redeploy."
            )
    ...
    def merge_datasets(self, datasets: list, strategy: str = "stack", join_key: str = None) -> dict:
        """
        Combine several parsed datasets into one.

        stack: append rows. Columns are unioned; missing values become None.
               A `source_file` column records which file each row came from.
        join:  merge on a shared key column present in every dataset.
        """
        if not datasets:
        ...
        for d in datasets:
            ...
            df["source_file"] = d.get("filename", "unknown")
            frames.append(df)

        if strategy == "join":
            if not join_key:
                # Auto-detect: a column present in every dataset
                common = set(frames[0].columns)
                for f in frames[1:]:
                    common &= set(f.columns)
                common.discard("source_file")
                if not common:
                    raise ValueError("No shared column found to join on. Use strategy='stack' instead.")
                # Prefer a column that looks like a key (id / code / name / date / month)
                preferred = [c for c in common if any(k in c.lower() for k in
                             ("id", "code", "key", "month", "date", "period", "name", "region"))]
                join_key = preferred[0] if preferred else sorted(common)[0]

            for f in frames:
                if join_key not in f.columns:
                    raise ValueError(f"Join key '{join_key}' is missing from one of the files.")

            merged = frames[0].drop(columns=["source_file"])
            for i, f in enumerate(frames[1:], 2):
                right = f.drop(columns=["source_file"])
                merged = merged.merge(right, on=join_key, how="outer", suffixes=("", f"_{i}"))
            merged = self._clean_frame(merged)
            return {
                "strategy": "join",
                "join_key": join_key,
                "records": self._to_records(merged),
                "columns": list(merged.columns.astype(str)),
                "row_count": len(merged),
                "col_count": len(merged.columns),
                "files_merged": [d.get("filename") for d in datasets],
            }
        ...
        return {
            ...
            "col_count": len(merged.columns),
            "files_merged": [d.get("filename") for d in datasets],
            "rows_per_file": {d.get("filename"): d["row_count"] for d in datasets},
        }
